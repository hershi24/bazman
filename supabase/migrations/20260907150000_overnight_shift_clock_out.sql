-- Overnight shifts: if clock-out is earlier than clock-in, treat it as the next calendar day.
...
BEGIN
  ...
  start_ts := COALESCE(new_in, rec.clock_in);
  IF new_out IS NOT NULL AND start_ts IS NOT NULL AND new_out <= start_ts THEN
    new_out := new_out + interval '1 day';
  END IF;
